...
export default async function AccountPage() {
    ...
    const ventures = user.ventures as Array<{ ventureName: string; status: string; linkedAt: string; thirdParty?: boolean }> | undefined
            ...
            <div className="account-page__header">
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div>
                        ...
                        <p>Manage your Realms & Beyond identity and connected ventures.</p>
                    </div>
            ...
            <div className="account-page__grid">
                ...
                <div className="account-page__section">
                    ...
                    {ventures && ventures.length > 0 ? (
                        ventures.map((v) => (
                            <div key={v.ventureName} className="account-page__row">
                                <span>{v.ventureName}{v.thirdParty ? ' (3rd party)' : ''}</span>
                                <span className="account-page__badge">{v.status}</span>
                            </div>
                        ))
                    ) : (
                        <p style={{ fontSize: '0.8rem', color: 'var(--text-muted-color)' }}>
                            No connected ventures yet.
                        </p>
                    )}
                </div>
            </div>
